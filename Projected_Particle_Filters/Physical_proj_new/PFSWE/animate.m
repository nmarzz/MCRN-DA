function animate(time,xnew,pars,parsanim,string)
...
[u_save,v_save,h_save]=swe_vectortodata(xnew,nx,ny);
%%Break up xnew into u,v,h
%u1=xnew(1:nx*ny);
%v1=xnew(nx*ny+1:2*nx*ny);
%h1=xnew(2*nx*ny+1:3*nx*ny);
%u_save=reshape(u1,nx,ny);
%v_save=reshape(v1,nx,ny);
%h_save=reshape(h1,nx,ny);
...
%for it = 1:noutput
  ...
  quiver(x_1000km(3:interval:end), y_1000km(3:interval:end), ...
         ...
         real(v(3:interval:end, 3:interval:end))','k');
  ...
  subplot(2,1,2);
  handle = image(x_1000km, y_1000km, real(vorticity'));
  set(handle,'CDataMapping','scaled');
  set(gca,'ydir','normal');
  caxis([-3 3].*1e-4);
